...
(defn handle-change [e data edit-key owner]
  (prn [:handle-change edit-key {:d @data :e-t-v (.. e -target -value)}])
  (om/transact! data edit-key (fn [_] (.. e -target -value))))

(defn end-edit [data edit-key text owner cb]
  (prn [:end-edit edit-key])
      ...
      (dom/div nil
        (om/build om-sync (:classes app)
          {:opts {:view classes-view
                  ...
                  :id-key :class/id
                  :on-success (fn [res tx-data] (prn [:om-sync-success res]))
                  :on-error
                  (fn [err tx-data]
                    (prn [:om-sync-error (keys tx-data) tx-data])
